...
bool State::encontrarLugar()
{
	...
	set<int> dondePuede;
	for (int i = 0; i < 9; i++)
	{ //recorro las filas y columnas
		for (int numero = 1; numero <= 9; numero++)
		{ //veo desde 1 a 9, la custion
			//con filas
			if (!numInFila(numero, i))
			{ //busco si existe el numero de arriba en la fila
				count = 0;
				dondePuede.clear();
				for (int k = 0; k < 9; k++)
				{ //si no existe, voy casilla por casilla en la fila
